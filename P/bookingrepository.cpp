...
bool BookingRepository::isRoomBookedInPeriod(int roomId, const QString& checkIn, const QString& checkOut, int excludeBookingId) {
    ...
    query.prepare("SELECT id FROM bookings WHERE room_id = :roomId AND "
                  "(LOWER(status) != 'скасовано' AND LOWER(status) != 'виписано') AND id != :excludeId AND "
...
QString BookingRepository::getBookedDatesForRoom(int roomId) {

    QSqlQuery query(DatabaseManager::getInstance()->getDB());
    query.prepare("SELECT check_in, check_out FROM bookings WHERE room_id = :roomId AND"
                  " status != 'Скасовано' AND status != 'Виписано' ORDER BY check_in");
    query.bindValue(":roomId", roomId);
    query.exec();

    QStringList bookedDates;
    while(query.next()) {
        QString checkIn = query.value("check_in").toString();
        QString checkOut = query.value("check_out").toString();
        bookedDates.append(QString("%1 по %2").arg(checkIn).arg(checkOut));
    }

    if (bookedDates.isEmpty()) {
        return "Наразі немає активних бронювань для цього номера.";
    }
